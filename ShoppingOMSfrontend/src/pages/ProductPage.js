...
const ProductPage = ({ input, setInput }) => {
    ...
    const applyFilters = () => {
        ...
        const inputFilter = products.filter((item) => {
            const matchSearch1 = item.itemName.toLowerCase().includes(input) || item.itemName.toUpperCase().includes(input) || item.itemName.includes(input);
            return matchSearch1
        });

        let filteredProducts = [...inputFilter];
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            {/* Sidebar */}
            <Sidebar
                categories={categories.filter((category) => category.isActive === 1)}
            ...
            >
                <Products filteredProducts={filteredProducts} addToWishlist={addToWishlist} removeFromWishlist={removeFromWishlist}
                    wishlists={wishlists} carts={carts} addToCart={addToCart} isWishlist={true} />
            </Box>
        </Box>
    );
};
